...
export default function WarrantyModal() {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 ${warrantyOpen ? '' : 'hidden'}`}>
      <div className={`absolute inset-0 bg-black/60 backdrop-blur-md overlay-base ${warrantyOpen ? 'overlay-visible' : ''}`} onClick={() => setWarrantyOpen(false)}></div>
      <div className={`bg-white w-full relative z-10 rounded-[28px] flex flex-col max-h-[90vh] border border-black/5 shadow-2xl overflow-hidden modal-content-animate modal-center modal-center-wide ${warrantyOpen ? 'open' : ''}`}>
