  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-center px-4" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 12px) + 16px)' }}>
      <div className="capsule-nav rounded-full flex items-center px-6 py-2 gap-4 w-full max-w-[300px] justify-between shadow-[0_20px_40px_rgba(0,0,0,0.12)]">
        <button onClick={() => navigateTo('home')} className="nav-pill-item flex flex-col items-center justify-center w-[52px] h-[52px] rounded-full text-gray-400 hover:text-[#1d1d1f] hover:bg-black/5">
        ...
        {currentUser ? (
          ...
          <button onClick={() => setAuthOpen(true)} className="nav-pill-item flex flex-col items-center justify-center w-[52px] h-[52px] rounded-full text-gray-400 hover:text-[#1d1d1f] hover:bg-black/5 cursor-pointer">
