...
import { Star, Shield, Zap, Eye, EyeOff } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center">
        <div className="flex flex-col items-center gap-5">
          <img src="/logo.webp" alt="" className="w-12 h-12 object-contain" style={{color:'transparent', animation:'gentlePulse 1.5s ease-in-out infinite'}} onError={(e) => { e.target.onerror = null; e.target.outerHTML = '<div class="w-12 h-12 bg-[#1d1d1f] rounded-[14px] flex items-center justify-center"><span class="text-white font-black text-xl">M</span></div>'; }} />
          <div className="w-[80px] h-[3px] bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-[#1d1d1f] rounded-full animate-apple-load"></div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex relative overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[48%] xl:w-[50%] bg-[#08080a] relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage:'radial-gradient(rgba(255,255,255,0.5) 1px, transparent 1px)', backgroundSize:'32px 32px'}}></div>
        </div>
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 xl:p-16 w-full">
          ...
          <Link href="/" className="flex items-center gap-3 group w-fit">
            ...
            <div>
              ...
              <div className="text-[9px] text-white/30 font-semibold tracking-[0.2em] mt-0.5">PREMIUM ACCOUNTS</div>
            </div>
          </Link>
          ...
          <div className="max-w-md">
            <h1 className="text-[42px] xl:text-[50px] font-black text-white tracking-tight leading-[1.05] mb-5">
              Trải nghiệm<br />
              <span className="bg-gradient-to-r from-violet-400 via-blue-400 to-emerald-400 bg-clip-text text-transparent">Premium</span> không giới hạn
            </h1>
            <p className="text-[15px] text-zinc-500 leading-relaxed font-medium max-w-sm">
              Kho tài khoản chính hãng với mức giá tốt nhất. Bảo hành trọn đời, giao hàng tự động, hỗ trợ 24/7.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            {[
              { icon: Star, label: '5.0', sub: 'Đánh giá', bg: 'bg-amber-500/10', text: 'text-amber-400' },
              { icon: Shield, label: 'Trọn đời', sub: 'Bảo hành', bg: 'bg-emerald-500/10', text: 'text-emerald-400' },
              { icon: Zap, label: 'Tự động', sub: 'Giao hàng', bg: 'bg-blue-500/10', text: 'text-blue-400' },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-2.5 bg-white/[0.03] border border-white/[0.06] rounded-2xl px-4 py-3">
      ...
      <div className="flex-1 flex flex-col bg-[#fafafa] relative">
        ...
        <div className="lg:hidden absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-[-15%] right-[-15%] w-[350px] h-[350px] bg-violet-200/20 rounded-full blur-[100px]"></div>
          <div className="absolute bottom-[-15%] left-[-15%] w-[300px] h-[300px] bg-blue-200/15 rounded-full blur-[100px]"></div>
        </div>

        {/* Mobile top bar */}
        <div className="lg:hidden flex items-center px-5 pt-5 relative z-10">
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12 relative z-10">
          <div className="w-full max-w-[400px]">
            ...
            <button
              ...
              className="w-full bg-white border border-black/[0.08] text-[#1d1d1f] py-3.5 rounded-2xl font-bold text-[13px] hover:border-black/15 hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-2.5 shadow-sm mb-6 disabled:opacity-60 active:scale-[0.98]"
            >
              {googleLoading ? (
            ...
            <form onSubmit={handleAuth} className="space-y-3">
              <div>
                ...
                <input type="email" name="email" autoComplete="username" placeholder="you@example.com" className="w-full px-4 py-3.5 rounded-2xl bg-white border border-black/[0.06] focus:border-black/15 focus:shadow-[0_0_0_4px_rgba(0,0,0,0.03)] transition-all duration-200 outline-none text-[13px] font-medium placeholder:text-gray-300" />
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block ml-1">Mật khẩu</label>
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} name="password" autoComplete={isLoginMode ? 'current-password' : 'new-password'} placeholder="Tối thiểu 6 ký tự" className="w-full px-4 py-3.5 pr-12 rounded-2xl bg-white border border-black/[0.06] focus:border-black/15 focus:shadow-[0_0_0_4px_rgba(0,0,0,0.03)] transition-all duration-200 outline-none text-[13px] font-medium placeholder:text-gray-300" />
              ...
              <button type="submit" disabled={loading} className="w-full bg-[#1d1d1f] text-white py-3.5 rounded-2xl font-bold text-[13px] hover:bg-black/80 active:scale-[0.98] transition-all duration-200 shadow-lg shadow-black/10 disabled:opacity-60 disabled:pointer-events-none !mt-5">
                {loading ? (
            ...
            <p className="text-center text-[13px] text-gray-500 font-medium mt-7">
              ...
              <button type="button" onClick={() => { setIsLoginMode(!isLoginMode); setShowPassword(false); }} className="font-bold text-[#1d1d1f] ml-1.5 hover:underline underline-offset-2">{isLoginMode ? 'Đăng ký ngay' : 'Đăng nhập'}</button>
            </p>

            <p className="text-center text-[11px] text-gray-400 mt-5 leading-relaxed">
              Bằng việc tiếp tục, bạn đồng ý với{' '}
              <Link href="/" className="underline underline-offset-2 hover:text-gray-600 transition-colors">Điều khoản</Link>
              {' '}và{' '}
